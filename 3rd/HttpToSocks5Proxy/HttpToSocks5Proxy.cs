...
namespace MihaZupan
{
    ...
    public class HttpToSocks5Proxy : IWebProxy
    {
        ...
        {
            set { dnsResolver = value ?? throw new ArgumentNullException(nameof(value)); }
        }
        ...
        public HttpToSocks5Proxy(
            string socks5Hostname,
            int socks5Port,
            string username,
            string password,
            int internalServerPort = 0
        )
            : this(
                new[] { new ProxyInfo(socks5Hostname, socks5Port, username, password) },
                internalServerPort
            ) { }
        ...
        {
            if (internalServerPort < 0 || internalServerPort > 65535)
                throw new ArgumentOutOfRangeException(nameof(internalServerPort));
            if (proxyList == null)
                throw new ArgumentNullException(nameof(proxyList));
                ...
                )
                {
                    try
                    {
                        ...
                        socks5Socket.Connect(
                            dnsResolver.TryResolve(ProxyList[0].Hostname),
                            ProxyList[0].Port
                        );
                    }
                    ...
                    if (success)
                    {
                        ...
                        for (int i = 0; i < ProxyList.Length - 1; i++)
                        {
                            ...
                            result = Socks5.TryCreateTunnel(
                                socks5Socket,
                                nextProxy.Hostname,
                                nextProxy.Port,
                                proxy,
                                ResolveHostnamesLocally ? dnsResolver : null
                            );
        ...
        )
        {
            ...
            List<string> headerLines = headerString
                .Split('\n')
            ...
            else
            {
                ...
                for (int i = 1; i < headerLines.Count; i++)
                {
                    ...
                    if (colon == -1)
                        continue; // Invalid header found (no colon separator) - skip it instead of aborting the connection
        ...
        )
        {
            ...
            while (
                !headersBuffer.ContainsDoubleNewLine(
                    Math.Max(0, offset - 3),
                    received,
                    out endOfHeader
                )
        ...
        private static void SendError(
            Socket socket,
            SocketConnectionResult error,
            string httpVersion = "HTTP/1.1 "
        ) => SendString(socket, ErrorResponseBuilder.Build(error, httpVersion));

        private static Socket CreateSocket() => new Socket(SocketType.Stream, ProtocolType.Tcp);

    ...
    class Vmess : ConfigerComponentController
    {
        ...
        JToken GetSettings()
        {
            ...
            if (serverMode)
            {
                ...
                vmess["settings"]["clients"][0]["level"] = VgcApis.Misc.Utils.Str2Int(level);
                vmess["settings"]["clients"][0]["alterId"] = VgcApis.Misc.Utils.Str2Int(altID);
            }
            else
            {
        ...
        {
            var root = Misc.Utils.GetConfigRoot(serverMode, v4Mode);
            var vmess = Misc.Utils.CreateJObject(root, GetSettings());
            try
            {

...
local parser = require('3rd/neolua/libs/luacheck/parser')
...
local function GetModuleProps(src, r)
    ...
    for k,v in pairs(src) do
        if Get(v, 1, 1, 1) == on and Get(v, 1, 1, "tag") == "Id"
           and Get(v, 2, 1, "tag") == "Table" 
        then
            -- table.dump(t, indent, header)
        ...
        elseif Get(v, 1, 1, 1, 1) == on
            and v.tag == "Set" and v[1][1].tag == "Index" 
            and Get(v, 1, 1, 2)
        then
            InsertOnce(r["props"], Get(v, 1, 1, 2, 1))
        end
    end
end
...
local function GetModuleInfo(ast, mName)
    ...
    for k, v in pairs(ast) do
        if v.tag == "Local"
            and Get(v, 1, 1, 1) == mName
            and Get(v, 2, 1) and type(v[2][1]) == "table"
        then
            -- print( DumpTable(v) )
            for sk, sv in pairs(v[2][1]) do
                if type(sv) == "table" and sv.tag == "Pair"
                    and Get(sv, 1, 1) == "string"
                then
                    -- print( DumpTable(sv) )
                    InsertOnce(r["props"], sv[1][1])
                end
        ...
        elseif v.tag == "Set"
            and Get(v, 1, 1, 1, 1) == mName
            and Get(v, 1, 1, 2, 1) and type(v[1][1][2][1]) == "string"
            and Get(v, 2, 1, "tag")
        then
            if v[2][1].tag ~= "Function" then
                -- props
                InsertOnce(r["props"], v[1][1][2][1])
            elseif Get(v, 2, 1, 2) -- function has source code
                and Get(v, 1, 1, 2, 1)
...
local function GetModuleName(ast)
    for i = #ast, 1, -1 do
        local v = ast[i]
        if Get(v, "tag") == "Return" and Get(v, 1, 1) then
            return v[1][1]
        end
    end
    return nil
...
local function analyzeRequirePath(v, r)
    ...
    if Get(v, 1, 1) == "require" and Get(v, 2, 1) then
        return "modules", v[2][1]
    elseif v.tag == "Call" 
        and Get(v, 1, 1, 1, 1) == "require" 
        and Get(v, 1, 1, 2)
    then
        return "modules", Get(v, 1, 1, 2, 1)
    end
    
    if Get(v, 1, 1, 1) == "string" 
        and Get(v, 1, 2, 1)
        and type(v[1][2][1]) == "string"
    then
...
local function analyzeOneRequire(var, val, r)
    local vt, vv = analyzeRequirePath(val, r)
    if vt == "modules" then
        local isTable, nv = analyzeName(var, ".")
...
local ut = require('3rd/neolua/libs/utils')
...
local testCodeWithModule = [[
local CoreServMgr = require('3rd/neolua/cmpts/CoreServsMgr')

local csm = CoreServMgr.new("hello.txt", nil)
...
local function testHugeModule()
    ...
    local txt = std.Sys:ReadAllText(file)
...
local function testAnalyzeCode(src)
    local r = anz.analyzeCode(src)
    local o = json.decode(r)
    print( DumpTable(o) )
end
...
-- testAnalyzeCode(testCodeWithModule)
-- testHugeModule()

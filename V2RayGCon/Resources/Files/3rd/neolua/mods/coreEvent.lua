...
local function RegEvHandler(self, coreServ, fn, evType)
    ...
    if coreServ == nil then
        et = GLOBAL_EV
        handle = std.Sys:RegisterGlobalEvent(self.mailbox, evType, evCode)
    else
        et = CORE_EV
        handle = std.Sys:RegisterCoreEvent(coreServ, self.mailbox, evType, evCode)
    end
...
function coreEv:RegEvClosing(coreServ, handler)
    return RegEvHandler(self, coreServ, handler,std.Sys.CoreEvClosing)
end
--]]

function coreEv:RegGlobalEvStart(handler)
    return RegEvHandler(self, nil, handler,std.Sys.CoreEvStart)
end

function coreEv:RegGlobalEvStop(handler)
    return RegEvHandler(self, nil, handler,std.Sys.CoreEvStop)
end

function coreEv:RegEvPropertyChanged(coreServ, handler)
    return RegEvHandler(self, coreServ, handler,std.Sys.CoreEvPropertyChanged)
end

function coreEv:RegEvStart(coreServ, handler)
    return RegEvHandler(self, coreServ, handler,std.Sys.CoreEvStart)
end

function coreEv:RegEvStop(coreServ, handler)
    return RegEvHandler(self, coreServ, handler,std.Sys.CoreEvStop)
end

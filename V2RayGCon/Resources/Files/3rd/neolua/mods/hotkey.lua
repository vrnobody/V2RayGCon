...
function hotkey:Reg(keyName, fn, hasAlt, hasCtrl, hasShift)
    ...
    local handle = std.Sys:RegisterHotKey(self.mailbox, evCode, keyName, hasAlt, hasCtrl, hasShift)
    if handle ~= nil then
...
function hotkey:Destroy()
    for idx, handle in ipairs(self.handles) do
        std.Sys:UnregisterHotKey(self.mailbox, handle)
    end
    self:ClearEvents()
    self.mailbox:Close()
end
...
function hotkey.new()
    ...
    local mailbox = std.Sys:ApplyRandomMailBox()

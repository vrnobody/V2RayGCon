...
local function CreateServer(self)
    ...
    local inbox = std.Sys:ApplyRandomMailBox()
    assert(inbox ~= nil, "Apply mail box failed!")
    
    local outbox = std.Sys:ApplyRandomMailBox()
    ...
    local serv = std.Sys:CreateHttpServer(self.url, self.inbox, self.outbox, self.source, self.allowCORS)
    return serv
end

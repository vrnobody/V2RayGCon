...
namespace V2RayGCon.Services.ShareLinkComponents.VeeCodecs
{
        ...
        IVeeDecoder
    {
        ...
        public string GetSupportedVersion() =>
           Models.VeeShareLinks.Ver0a.SupportedVersion();
        ...
        {
            var veeLink = new Models.VeeShareLinks.Ver0a(bytes);
        ...
        Models.VeeShareLinks.Ver0a Config2Vee(JObject config)
        {
        ...
        void ExtractTcpHttpSettings(JObject json, bool isUseV4, Models.VeeShareLinks.Ver0a vee)
        {
            try
            {
                var path = isUseV4 ?
                ...
                vee.streamParam2 = Misc.Utils.JArray2Str(path as JArray);
            }
            catch { }
            try
            {
                ...
                vee.streamParam3 = Misc.Utils.JArray2Str(hosts as JArray);
            }
            catch { }
        }

        Tuple<JObject, JToken> VeeToConfig(Models.VeeShareLinks.Ver0a vee)
        {
            if (vee == null)
            {
                return null;
        ...
        JToken GenStreamSetting(Models.VeeShareLinks.Ver0a vee)
        {
            // insert stream type
                ...
                switch (streamType)
                {
                    ...
                    case "tcp_http":
                        ...
                        streamToken["tcpSettings"]["header"]["request"]["path"] =
                            Misc.Utils.Str2JArray(string.IsNullOrEmpty(vee.streamParam2) ? "/" : vee.streamParam2);
                        streamToken["tcpSettings"]["header"]["request"]["headers"]["Host"] =
                            Misc.Utils.Str2JArray(vee.streamParam3);
                        break;
                    case "kcp":

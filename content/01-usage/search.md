---
title: "搜索功能"
date: 2020-02-01T16:12:15+08:00
draft: false
weight: 30
---

`v2.0.1+`  
{{< figure src="../../images/forms/demo_adv_filter.png" >}}

##### 简易搜索规则：
 * 忽略空格和大小写，模糊搜索，只匹配标题
 * `##`双井号开头，搜索除第一个`#`号以外的内容  
 * `#12`单个井号加数字，跳转到对应序号的服务器

##### 简易搜索示例：  
`a  B  C 1` 搜索标题包含`abc1`字符的服务器  
`##123` 搜索标题包含`#123`字符的服务器  
`#123` 跳转到序号为123的服务器  
`#`，`#0` 因为`#`后面的序号不存在，所以跳转到最后一个服务器  

##### 高级搜索格式：
`#<标签名> [not] <操作符> <参数1> <参数2> ...`
 * (v2.0.2+) not为可选参数，表示对后面的条件取反

| 文本型标签名 | 说明 |
| -------- | -------- |
| name | 服务器名字 |
| summary | 服务器摘要(协议@地址) |
| title | 服务器标题(序号+名字+摘要) |
| mark | 标记 |
| remark | 备注 |
| tag1-3 | 标签1-3 |
| core | 自定义core名 |
| selected | 是否已选中，内容是字符串 true 或者 false |

| 文本型操作符 | 说明 |
| -------- | -------- |
| is | 完全相同 |
| not | 完全不相同（v2.0.2将删除这项） |
| has | 包含 |
| hasnot | 不包含（v2.0.2将删除这项） |
| like | 模糊匹配（默认操作符） |
| unlike | 模糊匹配失败（v2.0.2将删除这项） |
| starts | 匹配开头 |
| ends | 匹配结尾 |

###### 示例：
`#mark is a B C` 搜索“标记”是“abc”的服务器。文本类型的参数会合并起来。  
`#mark abc` 搜索“mark”中出现“abc”三个字母的服务器。文本类型默认操作符是“like”。  
`#mk has abc` 因为“mk”可以匹配“mark”和“remark”两个标签名，他们任意一个包含“abc”都会搜到。  
`#sel not f` 搜索已勾选的服务器。相当于`#selected not like false`。  
`#name is ""` 搜索“名称”为空白的服务器。  
`#name has " "` 搜索“名称”中包含空格的服务器。  
`#name has """` 搜索“名称”中包含引号的服务器。  

| 数字型标签名 | 说明 |
| -------- | ------- |
| index | (v2.0.2+)序号 |
| latency | 测速延迟(ms)，没测速的是-1，超时是个很大很大的数 |
| upload | 上传量(MiB) |
| download | 下载量(MiB) |
| port | inbound端口号 |
| modify | 修改日期，格式：240230，年份可以省略 |

| 数字型操作符 | 说明 | 参数个数 |
| -------- | ------- | -------- |
| ! | 不等（v2.0.2将删除这项） | 1 |
| = | 相等（v2.0.2起，默认操作符） | 1 |
| < | 小于等于（v2.0.2这项改为仅小于） | 1 |
| > | 大于等于（v2.0.2这项改为仅大于） | 1 |
| ~ | 数值范围包括首尾 | 2 |

###### 示例：
`#idx 3` 序号为3的服务器。因为默认操作符是`=`，所以相当于`#index = 3`  
`#idx not > 10` 搜索序号小于等于10的服务器。  
`#latency > 100 300` 搜索测速结果大于（不包括）100ms的服务器。后面多余的参数300被忽略掉。  
`#latency ~ 1 1000` 搜索测速结果在[1ms, 1000ms]之间的服务器。  
`#latency ~ 123` 参数个数不足，搜索失败。  
`#modify = 0230` 搜索二月三十日导入的服务器。  
`#index` 参数不足，搜索失败。  


##### 组合搜索(v2.0.2+)

输入`#@`进入组合搜索模式。高级搜索规则可以用`&`，`|`，`!`互相组合。默认从左向右组合，可以添加`()`括号调整组合顺序。  

| 组合操作符 | 说明 | 左边 | 右边 | 结果 |
| -------- | ------- | ------- | -------- | ------- |
| `&` | 交集 | [a, b] | [b, c] | [b] |
| `\|` | 并集 | [a, b] | [b, c] | [a, b, c] |
| `!` | 补集 | [a, b] | [b, c] | [a] |

###### 示例：
`#@ #idx < 3 | #idx ~ 6 7` 搜索结果序号：[1, 2, 3, 6, 7]  
`#@ #smm starts trojan & #name is ""` 搜索trojan协议并且名字是空白的服务器  
`#@ #smm has .ws & (#idx < 10 ! #idx = 5)` 搜索除5号外前10个服务器中使用ws协议的服务器  

##### 快捷键
`Ctrl + F` 进入搜索框  
`Esc` 离开搜索框  

...
支持 `vless://...` 分享链接的 vcn 参数  
修复 Composer 插件无法拖放服务器的问题  
主窗口使用更激进的刷新策略
...
修复`CVE-2024-48510` ZIP 文件路径穿越漏洞
...
添加一个网页版客户瑞，详见 WebUI
修复 lua.modules.reader.lua 中的一个 bug  
...
支持 vmess 分享链接中的`sni`及`gRPC`配置项  
`选项`窗口添加 uTLS 指纹伪装配置项  
...
优化日志窗口刷新算法减少卡顿  
优化多屏体验  
Luna 添加`Misc:SetSubscriptionConfig(cfgStr)`
...
主窗口搜索时保留当前的选中状态  
删除 Luna 中的`Web:FindAllHrefs()`函数
...
vee 链接支持 v2ray/discussion [#513](https://github.com/v2ray/discussion/issues/513) 中提出的 socks outbound（测试中）
